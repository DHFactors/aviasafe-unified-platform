...
function bindEvents(signInWithEmailAndPassword, signOut) {
    ...
    loginBtn.addEventListener('click', async () => {
        ...
        try {
            // Firebase Auth validates credentials; tenant resolved from custom claims
        ...
        if (user) {
            user.getIdTokenResult().then((idTokenResult) => {
                if (idTokenResult.claims.role === 'AIRLINE_ADMIN' && idTokenResult.claims.tenant_id) {
                    ...
                    document.getElementById('authMessage').textContent = "Unauthorized. AIRLINE_ADMIN role required.";
...
async function fetchTenantData(collectionRef = null, getDocsRef = null, queryRef = null, orderByRef = null, limitRef = null) {
    ...
    try {
        // Dynamically import Firebase modules for Firestore query
